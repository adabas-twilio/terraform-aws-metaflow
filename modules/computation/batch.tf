# Batch compute environment for inline policies
resource "aws_batch_compute_environment" "this_inline" {
...
# Batch compute environment for independent policies
resource "aws_batch_compute_environment" "this_independent" {
  ...
  # On destroy, this avoids removing these policies below until compute environments are destroyed
  depends_on = [
    aws_iam_role_policy_attachment.batch_execution_iam_pass_role_attachment,
    aws_iam_role_policy_attachment.batch_execution_custom_access_attachment,
    aws_iam_role_policy_attachment.batch_execution_iam_custom_attachment,
    aws_iam_role_policy_attachment.batch_execution_ec2_custom_attachment,
  ]
...
resource "aws_batch_job_queue" "this" {
  ...
  compute_environment_order {
    order = 1
    compute_environment = var.use_inline_policies ? (
      aws_batch_compute_environment.this_inline[0].arn
      ) : (
      aws_batch_compute_environment.this_independent[0].arn
    )
  }
