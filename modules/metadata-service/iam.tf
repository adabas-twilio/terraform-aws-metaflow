...
# Inline IAM Policies for metadata_svc_ecs_task_role (when use_inline_policies = true)
...
# Independent IAM Policies for metadata_svc_ecs_task_role (when use_inline_policies = false)
resource "aws_iam_policy" "metadata_svc_s3_kms_policy" {
  count  = var.use_inline_policies ? 0 : 1
  name   = "${var.resource_prefix}metadata-ecs-task${var.resource_suffix}-s3-kms"
  policy = data.aws_iam_policy_document.s3_kms.json
  tags   = var.standard_tags
}

resource "aws_iam_policy" "metadata_svc_custom_s3_policy" {
  count  = var.use_inline_policies ? 0 : 1
  name   = "${var.resource_prefix}metadata-ecs-task${var.resource_suffix}-custom-s3"
  policy = data.aws_iam_policy_document.custom_s3_batch.json
  tags   = var.standard_tags
}

resource "aws_iam_policy" "metadata_svc_deny_presigned_policy" {
  count  = var.use_inline_policies ? 0 : 1
  name   = "${var.resource_prefix}metadata-ecs-task${var.resource_suffix}-deny-presigned"
  policy = data.aws_iam_policy_document.deny_presigned_batch.json
  tags   = var.standard_tags
}

# Policy Attachments for metadata_svc_ecs_task_role (when use_inline_policies = false)
resource "aws_iam_role_policy_attachment" "metadata_svc_s3_kms_attachment" {
  count      = var.use_inline_policies ? 0 : 1
  role       = aws_iam_role.metadata_svc_ecs_task_role.name
  policy_arn = aws_iam_policy.metadata_svc_s3_kms_policy[0].arn
}

resource "aws_iam_role_policy_attachment" "metadata_svc_custom_s3_attachment" {
  count      = var.use_inline_policies ? 0 : 1
  role       = aws_iam_role.metadata_svc_ecs_task_role.name
  policy_arn = aws_iam_policy.metadata_svc_custom_s3_policy[0].arn
}

resource "aws_iam_role_policy_attachment" "metadata_svc_deny_presigned_attachment" {
  count      = var.use_inline_policies ? 0 : 1
  role       = aws_iam_role.metadata_svc_ecs_task_role.name
  policy_arn = aws_iam_policy.metadata_svc_deny_presigned_policy[0].arn
}
